...
cname <- file.path("E:/Dropbox/Courses/DATA SCIENCE TRACK/10. Capstone Project", "Dataset", "split test")
length(dir(cname))
dir(cname)
...
docs <- tm_map(docs, toString, "\\.", "")
docs <- tm_map(docs, toString, "'€™", "'")
docs <- tm_map(docs, toString, "â???T", "'")
docs <- tm_map(docs, toString, "â???~œ", "'")
docs <- tm_map(docs, toString, "T", "")
docs <- tm_map(docs, toString, " T", "")

tonothing <- function(x, pattern) gsub(pattern, "", x) ##Replace char with nothing
docs <- tm_map(docs, tonothing, "â???o“")
...
##setwd("E:/Dropbox/Courses/DATA SCIENCE TRACK/10. Capstone Project/Dataset/Processed 2")
...
setwd("E:/Dropbox/Courses/DATA SCIENCE TRACK/10. Capstone Project/Dataset/split processed")
for(i in 1:150)
{
  filename <- paste(formatC(i, width = 3, format = "d", flag = "0"),".txt",sep="")#leading zeros
  writeLines(docs[[i]],filename)
}
